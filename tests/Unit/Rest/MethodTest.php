...
it('can use get request to get managed zones', function(){
   ...
   $response = $client->rest()->get('/' . env('GOOGLE_CLOUD_TEST_PROJECT_ID') . '/managedZones', []);
...
it('can use post request to create a new managed zone', function(){
    ...
    $response = $client->rest()->post('/' . env('GOOGLE_CLOUD_TEST_PROJECT_ID') . '/managedZones', $request_data);
    expect($response->status->code)->toBe(200);
});

it('can use delete request to delete a managed zone', function(){
    $client = new Glamstack\GoogleCloud\ApiClient('test');
    $response = $client->rest()->delete('/' . env('GOOGLE_CLOUD_TEST_PROJECT_ID') . '/managedZones/testing-zone-3');
